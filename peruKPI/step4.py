...
#设置需要从目标表的表读取数据
tablename_for_kpi='kpi_for_qcms1'
...
if minTime!='' and maxTime!='':#最大和最小时间均能找到
    ...
    for stsid in stsids:#遍历岸桥编号
        ...
        querysqlforstsno_task = f"SELECT * FROM {tablename_for_kpi} WHERE STS_NO = '{stsid}' and KEYTIME>='{minTime}' and KEYTIME<='{maxTime}' order by KEYTIME asc"  #
        ...
        if isinstance(filtered_dfs,pd.DataFrame):#能查询出来数据
            ...
            #         qctaskdelete = filtered_dfs.iloc[qcdeleteStart_Qcmsindexs[0]]  # 开始的某一行数据
            #         namefortask=f"""QCMS删除过的任务{qctaskdelete['TASK_ID']}信息，任务类型:{qctaskdelete['TASK_TYPE']},任务状态:{qctaskdelete['TASK_STATUS']},任务初始位置:{qctaskdelete['ORIG_WSLOC']},任务目的位置:{qctaskdelete['DEST_WS_LOC']}"""
            ...
            #     else:#表示没有找到删除过的信息，则需要从另外的角度去画任务持续图
            ...
            #         if qcend_Qcmsindexs != []:  # 表示end数据能查到，则找第一条数据，画出图
            ...
            matching_rows_for_parkstart = filtered_dfs[
                filtered_dfs['DATA_FROM'] == 'QCMSDB.QC_TROLLEY_INSTRUCTION.Park.START_TIME']
            # 从匹配的行中提取索引值
            ...
            for parkstartindex in parkStart_Qcmsindexs:#查询QCMSpark指令发送的所有值
                ...
                # 判断查询出来的数据是否为空
                # 从匹配的行中提取索引值
                qcparkStart_Qcmsindexs = matching_rows_for_parkends.index.tolist()  # [1,3]查询出来的是个列表
                if qcparkStart_Qcmsindexs!=[]:#表示end数据能查到，则找第一条数据，画出图
                    qcparkend = filtered_dfs.iloc[qcparkStart_Qcmsindexs[0]]  # 开始的某一行数据
                    namefortask=f"""QCMS删除过的任务{qctaskdelete['TASK_ID']}信息，任务类型:{qctaskdelete['TASK_TYPE']},任务状态:{qctaskdelete['TASK_STATUS']},任务初始位置:{qctaskdelete['ORIG_WSLOC']},任务目的位置:{qctaskdelete['DEST_WS_LOC']}"""
                    fig.add_trace(go.Scatter(x=[qctaskstart['KEYTIME'],qctaskstart['KEYTIME'],qctaskdelete['KEYTIME'],qctaskdelete['KEYTIME'],qctaskstart['KEYTIME']],y=[qcms_min,qcms_max,qcms_max,qcms_min,qcms_min],fill='toself',fillcolor=color_for_gantt_red,line=dict(color='black'),name=namefortask),row=1,col=1)
                    #计算持续时间
                    durationforqctask=qctaskdelete['KEYTIME']-qctaskstart['KEYTIME']
            ...
            # parkStart_Qcmsindexs = matching_rows_for_parkstart.index.tolist()#[1,3]查询出来的是个列表
            #
            # if len(parkStart_Qcmsindexs)!=0:#表示能找到Park指令
            #     # 查找任务中有无匹配的park end指令对应的值
            #     matching_rows_for_parkends = filtered_dfs[
            #         (filtered_dfs['PAIRED_VALUE'] == qctaskstart['PAIRED_VALUE']) &
            #         (filtered_dfs['KEYTIME'] >= qctaskstart['KEYTIME']) &
            #         (filtered_dfs['DATA_FROM'] == 'QCMSDB.QC_TOS_TASK.UNLOCK_TIME')]
            ...
            #             parkstart = filtered_dfs.iloc[parkStart_Qcmsindexs[park_index]]#第一行数据
            #             parkend = filtered_dfs.iloc[parkEnd_Qcmsindexs[park_index]]
            #             #同时需要知道更新到哪个子图中
        ...
        filenameFortime = str(minTime).replace('-', '').replace(' ', '').replace(':', '').replace('.', '')
        # #打入文件夹的开始时间
        ...
        output_path=f"./documents_vbt_id/岸桥{stsid}的整点时间内{filenameFortime}任务明细.html"
        # 提取目录路径
    ...
    # wholeHourTimeforStss=wholeHourTimeEnds(minTime,maxTime)#['2025-01-09 02:00:00', '2025-01-09 03:00:00', '2025-01-09 04:00:00']
    # # 判断得到整点时间的长度时多少
    # lengthWholeHourTimesforSts=len(wholeHourTimeforStss)
    # if lengthWholeHourTimesforSts!=0:#当得到整点时间不为0才可以进行下一步,画出整点时间内的数据
    #     for stsid in stsids:#遍历岸桥编号
    #         for wholeHourTimeforSts_index,wholeHourTimeforSts in enumerate(wholeHourTimeforStss):#遍历整点数据
    #             # # #创建要画的图
    #             fig = make_subplots(cols=1, rows=1, subplot_titles='')  # cols表示设备数量，rows表示最大任务数量
    #             if wholeHourTimeforSts_index==0:#表示只有一条整点数据，那么查询语句
    #                 #查询的目的是为了查询整点时间范围内当前岸桥到底有多少task_id
    #                 querysqlforstsno_task = f"SELECT * FROM {tablename_for_kpi} WHERE STS_NO = '{stsid}' and KEYTIME<='{wholeHourTimeforSts}' order by KEYTIME asc"  #
    #             else:#表示有多条整点数据，那么查询语句
    #                 #查询的目的是为了查询整点时间范围内当前岸桥到底有多少task_id
    #                 querysqlforstsno_task = f"SELECT * FROM {tablename_for_kpi} WHERE STS_NO = '{stsid}' and KEYTIME>='{wholeHourTimeforStss[wholeHourTimeforSts_index-1]}' and KEYTIME<='{wholeHourTimeforSts}' order by KEYTIME asc"  #
    #
    #             filtered_dfs=o.query(querysqlforstsno_task,t='df')#整点时间内的数据
    #             if isinstance(filtered_dfs,pd.DataFrame):#能查询出来数据
    #
